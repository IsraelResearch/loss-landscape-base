...
def crunch(surf_file, net, w, s, d, dataloader, loss_key, acc_key, comm, rank, args):
    ...
    for count, ind in tqdm(
        enumerate(inds), total=len(inds), desc="Computing loss values..."
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", default=False, help="use test data"
    )
    parser.add_argument(
        "--plot",
        action="store_true",
    ...
    if args.cuda:
        ...
        print(
            "Rank %d use GPU number %d (%d GPUs available) on %s"
        ...
        if args.plot and rank == 0:
            if args.y and args.proj_file:
                plot_2D.plot_contour_trajectory(
                    surf_file, dir_file, args.proj_file, "test_loss", args.show
                )
            elif args.y:
                plot_2D.plot_2d_contour(
                    surf_file, "test_loss", args.vmin, args.vmax, args.vlevel, args.show
                )
            else:
